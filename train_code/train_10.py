...
for train, test in sk.split(X, y):
    ...
    test_xgb = xlf.predict_proba(X_test)
    # test_lgb = llf.predict_proba(X_test)
    # test_cab = clf.predict_proba(X_test)
    # ans = test_xgb * ii + test_lgb * jj + test_cab * kk
    ans = test_xgb
    answers.append(np.argmax(ans, axis=1))
    answers.append(xlf.predict(X_test))
    # answers.append(llf.predict(X_test))
    # answers.append(clf.predict(X_test).reshape(1, 2000)[0])
...
print(df.mean())
# test_end_score     0.891170
# xgboost            0.882266
# lightgbm           0.876821
# catboost           0.845335
# weight_xgboost     0.410000
# weight_lightgbm    0.385000
# weight_catboost    0.205000
fina = []
for i in range(2000):
...
import datetime
    ...
    (r"D:\a_zhy\MachineLearning\game\submit\10\submit_" + datetime.datetime.now().strftime('%Y%m%d_%H%M%S') + ".csv"),
    header=None, index=None, encoding='utf-8')
print(end.head())
print(end['type'].value_counts() / 2000)
# width=0.25
