...
#     train = pd.merge(train, t, on='ship', how='left')
#
#     t = df.groupby('ship')['v'].agg({'v_work_num': lambda x:x[x.values > 0.5].count()}).reset_index()
#     train = pd.merge(train, t, on='ship', how='left')
#     t = df.groupby('ship')['v'].agg({'v_notwork_num':lambda x:(x[x.values <= 0.5]).count()}).reset_index()
#     train = pd.merge(train, t, on='ship', how='left')
#
#     t = df.groupby('ship')['v'].agg({'v_work_mean': lambda x:  (x[x.values>0.5]).mean()}).reset_index()
#     train = pd.merge(train, t, on='ship', how='left')
#     t = df.groupby('ship')['v'].agg({'v_work_std': lambda x: (x[x.values>0.5]).std()}).reset_index()
#     train = pd.merge(train, t, on='ship', how='left')
#     t = df.groupby('ship')['v'].agg({'v_work_skew': lambda x: (x[x.values>0.5]).skew()}).reset_index()
#     train = pd.merge(train, t, on='ship', how='left')
#     t = df.groupby('ship')['v'].agg({'v_work_sum': lambda x:  (x[x.values>0.5]).sum()}).reset_index()
#     train = pd.merge(train, t, on='ship', how='left')
#
...
#
#     t = df.groupby('ship')['v'].agg({'v_notwork_mean': lambda x:  (x[x.values<=0.5]).mean()}).reset_index()
#     train = pd.merge(train, t, on='ship', how='left')
#     t = df.groupby('ship')['v'].agg({'v_notwork_std': lambda x: (x[x.values<=0.5]).std()}).reset_index()
#     train = pd.merge(train, t, on='ship', how='left')
#     t = df.groupby('ship')['v'].agg({'v_notwork_skew': lambda x: (x[x.values <= 0.5]).skew()}).reset_index()
#     train = pd.merge(train, t, on='ship', how='left')
#     t = df.groupby('ship')['v'].agg({'v_notwork_sum': lambda x:  (x[x.values<=0.5]).sum()}).reset_index()
#     train = pd.merge(train, t, on='ship', how='left')
#
#     # t = df.groupby('ship')['v'].agg({'v_work': lambda x: [v for v in x if v>0.5]}).reset_index()
#     # t['v_work_mean'] = t['v_work'].mean()
...
#         train = pd.merge(train, t, on='ship', how='left')
#         t = df.groupby('ship')['v'].agg({'v_work_%d'%int(100*p) : lambda x: (x[x.values>0.5]).quantile(p)}).reset_index()
#         train = pd.merge(train, t, on='ship', how='left')
#         t = df.groupby('ship')['v'].agg({'v_notwork_%d'%int(100*p) : lambda x: (x[x.values<=0.5]).quantile(p)}).reset_index()
#         train = pd.merge(train, t, on='ship', how='left')
...
#     return train
def extract_feature(df, train):
    t = group_feature(df, 'ship', 'x', ['max', 'min', 'mean', 'var','std', 'skew', 'sum'])
    train = pd.merge(train, t, on='ship', how='left')

    t = group_feature(df, 'ship', 'x', ['count'])
    train = pd.merge(train, t, on='ship', how='left')

    t = group_feature(df, 'ship', 'y', ['max', 'min', 'mean','var', 'std', 'skew', 'sum'])
    train = pd.merge(train, t, on='ship', how='left')

    t = group_feature(df, 'ship', 'v', ['max', 'min', 'mean', 'var','std', 'skew', 'sum'])
    train = pd.merge(train, t, on='ship', how='left')

    t = group_feature(df, 'ship', 'd', ['max', 'min', 'mean', 'var','std', 'skew', 'sum'])
    train = pd.merge(train, t, on='ship', how='left')
    ...
    t = df.groupby('ship')['v'].agg({'v_work_mean': lambda x:  (x[x.values>0.5]).mean()}).reset_index()
    train = pd.merge(train, t, on='ship', how='left')
    t = df.groupby('ship')['v'].agg({'v_work_var': lambda x:  (x[x.values>0.5]).var()}).reset_index()
    train = pd.merge(train, t, on='ship', how='left')
    t = df.groupby('ship')['v'].agg({'v_work_std': lambda x: (x[x.values>0.5]).std()}).reset_index()
    train = pd.merge(train, t, on='ship', how='left')
    t = df.groupby('ship')['v'].agg({'v_work_skew': lambda x: (x[x.values>0.5]).skew()}).reset_index()
    ...
    t = df.groupby('ship')['v'].agg({'v_notwork_mean': lambda x:  (x[x.values<=0.5]).mean()}).reset_index()
    train = pd.merge(train, t, on='ship', how='left')
    t = df.groupby('ship')['v'].agg({'v_notwork_var': lambda x:  (x[x.values<=0.5]).var()}).reset_index()
    train = pd.merge(train, t, on='ship', how='left')
    t = df.groupby('ship')['v'].agg({'v_notwork_std': lambda x: (x[x.values<=0.5]).std()}).reset_index()
    train = pd.merge(train, t, on='ship', how='left')
    t = df.groupby('ship')['v'].agg({'v_notwork_skew': lambda x: (x[x.values <= 0.5]).skew()}).reset_index()
    ...
    t = group_feature(df, 'ship', 'x_preq', ['max', 'min', 'mean','var', 'std', 'skew', 'sum'])
    train = pd.merge(train, t, on='ship', how='left')

    t = df.groupby('ship')['x_preq'].agg({'x_preq_argmax': lambda x: np.argmax(x)}).reset_index()
    train = pd.merge(train, t, on='ship', how='left')

    t = group_feature(df, 'ship', 'y_preq', ['max', 'min', 'mean', 'var','std', 'skew', 'sum'])
    train = pd.merge(train, t, on='ship', how='left')
    t = df.groupby('ship')['y_preq'].agg({'y_preq_argmax': lambda x: np.argmax(x)}).reset_index()
    train = pd.merge(train, t, on='ship', how='left')

    t = group_feature(df, 'ship', 'v_preq', ['max', 'min', 'mean','var', 'std', 'skew', 'sum'])
    train = pd.merge(train, t, on='ship', how='left')
    t = df.groupby('ship')['v_preq'].agg({'v_preq_argmax': lambda x: np.argmax(x)}).reset_index()
    train = pd.merge(train, t, on='ship', how='left')

    t = group_feature(df, 'ship', 'd_preq', ['max', 'min', 'mean','var', 'std', 'skew', 'sum'])
    train = pd.merge(train, t, on='ship', how='left')
    t = df.groupby('ship')['d_preq'].agg({'d_preq_argmax': lambda x: np.argmax(x)}).reset_index()
    train = pd.merge(train, t, on='ship', how='left')
...
def predict_fun(X, train=True, alpha=None):
    alpha_all = np.linspace(0.0001,0.0003,num=10)
...
print('best_alpha:{} oof_f1:{}'.format(alpha_best,metrics.f1_score(oof_best, y, average='macro')))
# best_alpha:0.0001 oof_f1:0.8490382913252406
# best_alpha:0.0001 oof_f1:0.8891346976307993
# best_alpha:0.0001 oof_f1:0.8729079928334825
...
sub.to_csv((r"D:\a_zhy\MachineLearning\game\submit\6\submit_" +
...
print(df)
# df.to_csv('catboost_feature_importance_6_3.csv',encoding='utf-8')
